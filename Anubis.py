...
class UI(QMainWindow):
    ...
    def intUI(self):
        ...
        languageOption = menu.addMenu('Language')
        Fexecute = menu.addMenu('Fast Execute')
...
#        Port.addAction(Port_Action)
        ...
        # execution Action
        FexecuteAction = QAction("Fexecute", self)
        FexecuteAction.triggered.connect(self.Execute)
        Fexecute.addAction(FexecuteAction)

        pythonAction = QAction('Python', self)
        pythonAction.triggered.connect(self.python_analyzer)
        csharpAction = QAction('C#', self)
        csharpAction.triggered.connect(self.csharp_analyzer)

        languageOption.addAction(pythonAction)
        languageOption.addAction(csharpAction)
...
#            ide.upload_file(self.portNo)
    ...
    ##Main Fast Execution Function
    def Execute(self):
        func = text.toPlainText()
        if exec(func) is None:
            text2.append("The function is correctly executed.")
        lines = func.splitlines()
        for line in lines:
            if "def" in line:
                leftbracket = line.find("(") + 1
                rightbracket = line.find(")")
                newLine = line[leftbracket:rightbracket]
                newLine = newLine.replace(" ", "")
                params = newLine.split(",")
                i = line.find("def") + 4
                j = line.find("(")
                outputString = "Output = " + line[i:j] + "("
                if params[0] != '':
                    inParams, doneEntering = QInputDialog.getText(self, "Parameters","Enter parameters in the following format x,y")
                    if doneEntering and inParams != '':
                        input_parameters = inParams.split(",")
                    for param in input_parameters:
                        outputString = outputString + param + ","
                    outputString = outputString[:-1]
                outputString = outputString + ")"
                exec(outputString)
                exec("text2.append('The resulted output is: ')")
